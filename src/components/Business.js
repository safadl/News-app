...
class Business extends React.Component{
    ...
    makeRemoteRequest = _.debounce(() => {
        ...
        getArticlesH('business', this.state.country).then(data=>{
            this.setState({
...
<ActivityIndicator style={{margin:Dimensions.get('window').height/4}} animating={this.state.isLoading} color='#ff5f40' size='large'/>
        </View>
    ) : 
    ( 
        <ScrollView>
...
<Content style={{backgroundColor:this.props.color? "#000000" : 'white' }}>
{view}
</Content></Container> 
            // </View>
        )
